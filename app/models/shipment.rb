...
class Shipment
    ...
    class << self
      def all
        results = CassandraClient.execute('SELECT * FROM my_keyspace.shipments')
      ...
      def find(id)
        ...
        statement = CassandraClient.prepare('SELECT * FROM my_keyspace.shipments WHERE id = ?')
      ...
      def create(attributes)
        ...
        statement = CassandraClient.prepare('INSERT INTO my_keyspace.shipments (id, description, containers, origin_room, destination_room) VALUES (?, ?, ?, ?, ?)')
      ...
      def update(id, attributes)
        uuid = Cassandra::Uuid.new(id) # Convert string id to Cassandra::Uuid
        statement = CassandraClient.prepare('UPDATE my_keyspace.shipments SET description = ?, containers = ?, origin_room = ?, destination_room = ? WHERE id = ?')
      ...
      def destroy(id)
        uuid = Cassandra::Uuid.new(id) # Convert string id to Cassandra::Uuid
        statement = CassandraClient.prepare('DELETE FROM my_keyspace.shipments WHERE id = ?')
