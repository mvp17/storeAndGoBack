class WorkerTask
    ...
    class << self
        ...
        def create(attributes)
            ...
            statement = CassandraClient.prepare('INSERT INTO my_keyspace.worker_tasks (id, type, description, room, detail, status, date) VALUES (?, ?, ?, ?, ?, ?, ?)')
        ...
        def update(id, attributes)
            uuid = Cassandra::Uuid.new(id)
            statement = CassandraClient.prepare('UPDATE my_keyspace.worker_tasks SET type = ?, description = ?, room = ?, detail = ?, status = ?, date = ? WHERE id = ?')
